...
def test_firmware_sends_compact_audio_before_text_only_fallback():
    src = read(EMAIL_CPP)
    # On-device logs showed the ESP32-C6 could no longer open TLS for a 216 KB
    # question-only payload after the Realtime leg. Keep the relay body small:
    # compact answer+question first, compact question-only second, text-only last.
    assert "EmailPayloadMode::kCompact8k" in src
    assert "EmailPayloadMode::kQuestionOnly8k" in src
    assert "EmailPayloadMode::kTextOnly" in src
    assert "send_payload(EmailPayloadMode::kCompact8k" in src
    assert "send_payload(EmailPayloadMode::kQuestionOnly8k" in src
    assert "send_payload(EmailPayloadMode::kTextOnly" in src
    assert src.index("send_payload(EmailPayloadMode::kCompact8k") < src.index(
        "send_payload(EmailPayloadMode::kQuestionOnly8k"
    )
    assert src.index("send_payload(EmailPayloadMode::kQuestionOnly8k") < src.index(
        "send_payload(EmailPayloadMode::kTextOnly"
    )


def test_firmware_downsamples_question_and_answer_for_relay():
    src = read(EMAIL_CPP)
    # Email review copies are not the local playback files. The question and
    # answer WAVs must both be downsampled and capped before the relay POST.
    ...
    assert "BuildPcm16WavHeader(compact_question_hdr" in src
    assert "BuildPcm16WavHeader(compact_ans_hdr" in src
